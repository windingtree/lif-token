...
contract('LifToken Crowdsale', function(accounts) {
  ...
  it("Should simulate a crowdsale of 7m tokens with three dutch auction stages, using future discount and distribute 3M of the tokens using futurePayments", async function() {
    ...
    await token.addCrowdsaleStage(startBlock, endBlock, web3.toWei(7, 'ether'), 10, web3.toWei(0.2, 'ether'), web3.toWei(10000000, 'ether'), web3.toWei(30000000, 'ether'), 2875000, 0);
    ...
    startBlock = currentBlock+10;
    endBlock = currentBlock+110;
    await token.addCrowdsaleStage(startBlock, endBlock, web3.toWei(10, 'ether'), 5, web3.toWei(0.1, 'ether'), web3.toWei(10000000, 'ether'), web3.toWei(30000000, 'ether'), 875000, 0);
    ...
    assert.equal(parseFloat(thirdDuctchAuction[5]), web3.toWei(10000000, 'ether'));
    assert.equal(parseFloat(maxSupply), 10000000);
    await help.waitToBlock(startBlock, accounts);
    ...
    assert.equal(parseFloat(thirdAuctionEnded[5]), web3.toWei(10000000, 'ether'));
    assert.equal(parseFloat(thirdAuctionEnded[6]), web3.toWei(30000000, 'ether'));
    assert.equal(parseFloat(thirdAuctionEnded[7]), 875000);
    assert.equal(parseFloat(thirdAuctionEnded[8]), 0);
    assert.equal(help.toEther(thirdAuctionEnded[9]), 0);
    assert.equal(help.parseBalance(thirdAuctionEnded[10]), help.parseBalance(totalWeiSent));
    assert.equal(parseFloat(thirdAuctionEnded[11]), totalTokensBought);
    assert.equal(parseFloat(thirdAuctionEnded[12]), lastPrice);
    assert.equal(parseFloat(thirdAuctionEnded[13]), 3);
    ...
    // Check all final values
    await help.checkValues(token, accounts, 0, 10000000, 0, [875000, 700000, 500000, 700000, 500000]);
  });
