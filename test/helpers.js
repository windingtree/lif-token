...
let gasPrice;
if (isNaN(gasPriceFromEnv)) { gasPrice = new BigNumber(20000000000); } else { gasPrice = new BigNumber(gasPriceFromEnv); }
...
module.exports = {
  ...
  checkToken: async function (token, accounts, totalSupply, balances) {
    ...
    if (totalSupply) { assert.equal(this.lifWei2Lif(parseFloat(tokenTotalSupply)), totalSupply); }
    if (balances) {
